...
- 本地个人工作台：读取 Obsidian 中的任务、项目、协作人角色卡和复盘，默认只读；
- 任务交付窗口：用 `start_on`、`due_on`、`scheduled_for`、`review_on` 在状态看板、项目看板和连续任务日历中呈现排期；
- 报告阅读器：为未来报告显示冻结指标、周期元数据、项目筛选、目录和证据标签；历史报告保持原样，不补推数字；
- 协作人目录：只读取 `type: topic` / `topic_kind: collaborator_reference` 的稳定角色卡，写入开关开启时才可新建或编辑稳定字段；
