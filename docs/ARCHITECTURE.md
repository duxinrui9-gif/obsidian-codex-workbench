...
1. **Mission Control** at the repository root reads one local Obsidian Vault. It is a dashboard and a narrow task/project writer, not a source-of-truth database. Active task delivery windows are rendered as inclusive calendar ranges; date roles are merged per task and day without changing task state.
