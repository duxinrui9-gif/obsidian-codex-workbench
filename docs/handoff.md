...
远端 `main` 包含当前工作台功能；`v0.2.0` 是本轮待发布版本，`v0.1.0` 保留为已发布回滚点。应用从仓库根目录以 `pnpm dev` 启动，默认只读并只绑定 `127.0.0.1`。它读取用户明确配置的 Vault，可从模板创建项目，并安全创建、更新和流转 `05_Review/Actions` 中的任务卡。Aldrich 已改为本地 WOFF2，不再请求第三方字体资源。
