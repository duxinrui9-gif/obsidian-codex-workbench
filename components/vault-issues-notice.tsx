...
export function VaultIssuesNotice({ issues, vaultName }: { issues: VaultIssue[]; vaultName: string }) {
  ...
  return <details className="vault-issues"><summary>数据降级：{issues.length} 个 Vault 文件已跳过，其他内容仍可使用</summary><ul>{issues.map((issue) => <li key={`${issue.kind}-${issue.relativePath}`}><span>{KIND[issue.kind]} · {issue.code}</span><a href={obsidianOpenUri(vaultName, issue.relativePath)} target="_blank" rel="noreferrer">{issue.relativePath}</a><small>{issue.message}</small></li>)}</ul></details>;
}
