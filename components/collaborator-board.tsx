...
export function CollaboratorBoard({ projects, writeEnabled }: { projects: ProjectSummary[]; writeEnabled: boolean }) {
  ...
  return <div className="collaborator-board">
    ...
    <CollaboratorDrawer item={selected} writeEnabled={writeEnabled} onClose={() => setSelected(null)} onSaved={(item) => { setData((current) => current ? { ...current, collaborators: current.collaborators.map((value) => value.id === item.id ? item : value) } : current); setSelected(item); }} />
  </div>;
}
...
function CollaboratorDrawer({ item, writeEnabled, onClose, onSaved }: { item: CollaboratorRecord | null; writeEnabled: boolean; onClose: () => void; onSaved: (item: CollaboratorRecord) => void }) {
