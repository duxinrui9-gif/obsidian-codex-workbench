...
import { deliverySignals, taskDateSummary, type DeliverySignals, type TaskBoardFilter } from "@/lib/task-board";
...
export function MissionControl() {
  ...
  const [taskBoardResetKey, setTaskBoardResetKey] = useState(0);
  ...
  return (
    <main className={`shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
      ...
      <aside className="sidebar" aria-label="主导航">
        ...
        <nav>{NAV.map((item) => <button key={item.id} title={item.label} className={view === item.id ? "nav-active" : ""} onClick={() => { setView(item.id); if (item.id === "tasks") { setTaskFilter(null); setTaskBoardResetKey((current) => current + 1); } }} aria-current={view === item.id ? "page" : undefined}><CyberIcon name={item.icon} /><span className="nav-label">{item.label}</span></button>)}</nav>
      ...
      <section className="workspace">
        ...
        <section id="mission-main" className="mission-main">
          ...
          {!loading && !error && view === "tasks" ? <TaskBoard key={`${taskFilter ?? "all"}-${taskBoardResetKey}`} actions={actions} onOpen={setSelected} filter={taskFilter} onClearFilter={() => setTaskFilter(null)} /> : null}
          {!loading && !error && view === "projects" ? <ProjectBoard projects={projects} writeEnabled={writeEnabled} onOpen={setSelected} onCreate={() => setNewProjectOpen(true)} onNewAction={(project) => { setNewActionProject(project); setNewOpen(true); }} onTransitioned={(project) => { setProjects((items) => [project, ...items.filter((item) => item.id !== project.id)]); void refresh(true); }} /> : null}
