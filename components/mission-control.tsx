...
function TaskRow({ action, onOpen }: { action: ActionRecord; onOpen: (action: ActionRecord) => void }) {
  return (
    <button className="task-row" onClick={() => onOpen(action)} type="button">
      ...
      <span className="task-date">{action.scheduledFor || action.reviewOn || "未排期"}</span>
    </button>
  );
}
  ...
  return (
    <main className={`shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
      ...
      <aside className="sidebar" aria-label="主导航">
        ...
        <nav>{NAV.map((item) => <button key={item.id} title={item.label} className={view === item.id ? "nav-active" : ""} onClick={() => setView(item.id)} aria-current={view === item.id ? "page" : undefined}><CyberIcon name={item.icon} /><span className="nav-label">{item.label}</span></button>)}</nav>
      ...
      <section className="workspace">
        ...
        <section id="mission-main" className="mission-main">
          ...
          {!loading && !error && view === "command" ? <CommandDeck inProgress={inProgress} todayTasks={todayTasks} overdue={overdue} waiting={waiting} review={review} backlog={backlog} projects={projects} onOpen={setSelected} onViewChange={setView} /> : null}
...
function CommandDeck({ inProgress, todayTasks, overdue, waiting, review, backlog, projects, onOpen, onViewChange }: { inProgress: ActionRecord[]; todayTasks: ActionRecord[]; overdue: ActionRecord[]; waiting: ActionRecord[]; review: ActionRecord[]; backlog: ActionRecord[]; projects: ProjectSummary[]; onOpen: (action: ActionRecord) => void; onViewChange: (view: View) => void }) {
  const next = inProgress[0] ?? todayTasks[0] ?? overdue[0];
  return <div className="command-grid">
    <section className="panel execution-panel"><div className="panel-head"><div><p className="eyebrow">FLIGHT PATH / EXECUTION</p><h2>推进舱</h2></div><span className="counter">{inProgress.length + todayTasks.length} ACTIVE</span></div>{next ? <button className="next-action" onClick={() => onOpen(next)}><CyberIcon name="next-action" className="next-action-icon" /><span><small>NEXT ACTION</small><strong>{next.title}</strong><em>{next.nextAction}</em></span><StatePill action={next} /></button> : <div className="empty-state"><strong>推进舱暂时清空</strong><span>把一张事项排入今天，或从 Backlog 启动一个任务。</span></div>}<div className="feed"><div className="feed-head"><span>进行中</span><span>{inProgress.length}</span></div>{inProgress.slice(0, 4).map((action) => <TaskRow key={action.id} action={action} onOpen={onOpen} />)}{!inProgress.length ? <p className="quiet">没有进行中的任务。</p> : null}<div className="feed-head"><span>今日待执行</span><span>{todayTasks.length}</span></div>{todayTasks.slice(0, 5).map((action) => <TaskRow key={action.id} action={action} onOpen={onOpen} />)}{!todayTasks.length ? <p className="quiet">今天还没有排期事项。</p> : null}</div></section>
    <aside className="risk-stack"><section className="panel risk-panel"><div className="panel-head"><div><p className="eyebrow">SYSTEM WATCH / RISK</p><h2><CyberIcon name="risk-watch" />风险舱</h2></div><button className="text-button" onClick={() => onViewChange("tasks")}>打开看板</button></div><div className="risk-matrix"><button onClick={() => onViewChange("tasks")}><CyberIcon name="overdue" /><span className="risk-num danger">{overdue.length}</span><small>逾期复查</small></button><button onClick={() => onViewChange("tasks")}><CyberIcon name="state-waiting" /><span className="risk-num wait">{waiting.length}</span><small>等待反馈</small></button><button onClick={() => onViewChange("tasks")}><CyberIcon name="state-review" /><span className="risk-num review">{review.length}</span><small>待确认</small></button><button onClick={() => onViewChange("tasks")}><CyberIcon name="state-backlog" /><span className="risk-num backlog">{backlog.length}</span><small>Backlog</small></button></div>{overdue.slice(0, 3).map((action) => <TaskRow key={action.id} action={action} onOpen={onOpen} />)}{!overdue.length ? <p className="quiet">没有逾期复查。系统运行平稳。</p> : null}</section><section className="panel project-radar"><div className="panel-head"><div><p className="eyebrow">PROJECT RADAR</p><h2><CyberIcon name="project-radar" />项目雷达</h2></div><button className="text-button" onClick={() => onViewChange("projects")}>全部项目</button></div>{projects.filter((project) => project.activeCount).slice(0, 5).map((project) => <div className="radar-row" key={project.name}><span className="radar-pin" /><strong>{project.name}</strong><span>{project.activeCount} TASKS</span><em>{project.overdueCount ? `${project.overdueCount} OVERDUE` : project.inProgressCount ? `${project.inProgressCount} RUNNING` : "STANDBY"}</em></div>)}</section></aside>
  </div>;
}
...
function ProjectBoard({ projects, writeEnabled, onOpen, onCreate, onNewAction }: { projects: ProjectSummary[]; writeEnabled: boolean; onOpen: (action: ActionRecord) => void; onCreate: () => void; onNewAction: (project: string) => void }) {
  ...
  return <div className="project-board">
    ...
    <div className="project-view-tabs" role="tablist" aria-label="项目范围"><button id="project-active-tab" role="tab" aria-selected={tab === "active"} aria-controls="project-active-panel" tabIndex={tab === "active" ? 0 : -1} className={tab === "active" ? "active" : ""} onClick={() => setTab("active")}>进行中</button><button id="project-ended-tab" role="tab" aria-selected={tab === "archived"} aria-controls="project-ended-panel" tabIndex={tab === "archived" ? 0 : -1} className={tab === "archived" ? "active" : ""} onClick={() => setTab("archived")}>已结束</button></div>
    <div id={tab === "active" ? "project-active-panel" : "project-ended-panel"} role="tabpanel" aria-labelledby={tab === "active" ? "project-active-tab" : "project-ended-tab"} className="project-grid">
