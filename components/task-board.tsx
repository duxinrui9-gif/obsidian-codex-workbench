...
import { actionProject, filterTaskBoardActions, groupArchivedActions, groupTaskDateEntries, preferredTaskCalendarDate, TASK_BOARD_FILTER_LABEL, TASK_DATE_KINDS, taskDateEntries, taskDateSummary, type TaskBoardFilter, type TaskDateKind } from "@/lib/task-board";
...
function StatusBoard({ actions, onOpen, filter, onClearFilter }: { actions: ActionRecord[]; onOpen: (action: ActionRecord) => void; filter: TaskBoardFilter | null; onClearFilter: () => void }) {
...
export function TaskBoard({ actions, onOpen, filter = null, onClearFilter }: { actions: ActionRecord[]; onOpen: (action: ActionRecord) => void; filter?: TaskBoardFilter | null; onClearFilter?: () => void }) {
  ...
  const filteredActive = filterTaskBoardActions(active, filter, hkDate());
  const views: TaskView[] = ["status", "calendar", "completed"];
  const selectByKey = (event: KeyboardEvent<HTMLButtonElement>, current: TaskView) => { const next = rovingTabIndex(views.indexOf(current), event.key, views.length); if (next !== null) { event.preventDefault(); setView(views[next]); requestAnimationFrame(() => document.getElementById(`task-tab-${views[next]}`)?.focus()); } };
