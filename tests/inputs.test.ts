...
import { parseActionPatch, parseCollaboratorPatch, parseCreateAction, parseCreateCollaborator, parseCreateProject, parseProjectTransition, parseTransition } from "../lib/inputs";
...
describe("runtime request parsing", () => {
  ...
  it("rejects missing fields, wrong arrays, and invalid enums", async () => {
    ...
    await expect(parseProjectTransition(request('{"expectedVersion":"v","transition":"skip"}'))).rejects.toMatchObject({ status: 422, code: "INVALID_INPUT" });
  ...
  it("keeps a valid write payload compatible with the existing API", async () => {
    ...
    await expect(parseProjectTransition(request('{"expectedVersion":"v","transition":"archive"}'))).resolves.toEqual({ expectedVersion: "v", transition: "archive" });
  });
});
